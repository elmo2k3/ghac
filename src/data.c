...
void addGraphData(struct _graph_data *graph, int modul, int sensor)
{
	...
	if (!mysql_real_connect(mysql_connection, config.graph_host, config.graph_user, config.graph_password, config.graph_database, 0, NULL, 0))
	{
	...
	if(modul==4)
	{
		mysql_connection = mysql_init(NULL);
		mysql_options(mysql_connection, MYSQL_OPT_COMPRESS, 0);
		if (!mysql_real_connect(mysql_connection, config.graph_host, config.graph_user, config.graph_password, config.graph_database_ws2000, 0, NULL, 0))
		{
			fprintf(stderr, "%s\n", mysql_error(mysql_connection));
			exit(0);
		}
